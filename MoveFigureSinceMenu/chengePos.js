...
function init()
{
    typeFigureID = document.getElementById("typeFigureID");
...
function save()
{
    var sqJson = JSON.stringify(sqlist);
    localStorage.setItem("Datakey",sqJson);
}
function load()
{
    clearCanv();
    var tempSq = localStorage.getItem("Datakey");
    sqlist = JSON.parse(tempSq);
    display();
}
...
function setColor()
{
    moveSq.color = document.getElementById("PalitraBleat").value;
    display();
};
function sizeL(val)
{
    moveSq.lineWidth = document.getElementById("TrackBar").value;
    display();
};
function checkFigure(type)
{
    if (type>0&&type<4)
    {
        moveSq.type = type;
...
function  mouseReleased(e)
{
    ...
    if(!isCheked)                                   // Add square in array
    {
        ...
        if((height < 0) && (width < 0))
        {
            height = -height;
            width  = -width;
            sq = new  Square(x - width, y - height, width,height,typeFigureID.value);
        }
        else if(width<0)
        {
            width = -width;
            sq = new  Square(x - width, y, width, height,typeFigureID.value);
        }
        else if(height<0)
        {
            height = -height;
            sq = new  Square(x, y-height, width, height,typeFigureID.value);
        }
        else
        {
            sq = new  Square(x, y, width, height,typeFigureID.value);
        }
        sqlist.push(sq);
    }
    NullAllVar();
